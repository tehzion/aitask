  ...
  return (
    <div className={pageShell}>
      <PageHeader
        ...
        meta={<><StatusChip tone={activePlan?.status === 'Active' ? 'emerald' : 'slate'}>{activePlan?.status || 'No active service'}</StatusChip>{currentCycle && <span>{formatLocalizedDate(parseOptionalDate(currentCycle.periodStart)!, locale)} – {formatLocalizedDate(parseOptionalDate(currentCycle.periodEnd)!, locale)}</span>}</>}
      />
      ...
      {tab === 'overview' && (
        <div id={`${TABS_ID}-panel-overview`} role="tabpanel" aria-labelledby={`${TABS_ID}-tab-overview`} tabIndex={0} className="grid scroll-mt-36 gap-5 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/35 xl:grid-cols-[minmax(0,1.55fr)_minmax(280px,.75fr)]">
          <Surface variant="inset" className="p-6 sm:p-8">
            ...
            <div className="mt-5 flex flex-wrap items-end justify-between gap-4"><div><p className="calm-number text-5xl font-semibold tracking-tight text-ink">{cycleCompletion}%</p><p className="mt-2 text-sm text-muted">{currentDeliverables.length ? <>{deliveredCount}/{currentDeliverables.length} {t('delivered this cycle')}</> : t('No published deliverables yet')}</p></div>{currentCycle && <StatusChip tone="emerald">{currentCycle.status}</StatusChip>}</div>
            <ProgressBar className="mt-7" value={deliveredCount} max={Math.max(currentDeliverables.length, 1)} label="Current cycle" />
