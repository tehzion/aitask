...
const ModalShell: React.FC<ModalShellProps> = ({
  ...
  return createPortal(
    <div
      className={cn('fixed inset-0 z-[100] flex items-center justify-center bg-scrim/60 p-3 backdrop-blur-[2px] sm:p-4', overlayClassName)}
      onMouseDown={event => {
    ...
    >
      <div
        ...
        tabIndex={-1}
        className={cn(
          'animate-modal-enter flex max-h-[calc(100dvh-1.5rem)] w-full flex-col overflow-hidden rounded-panel border border-line bg-surface text-ink shadow-float outline-none sm:max-h-[90vh]',
          panelClassName,
        )}
      >
        {children}
