-- Require new projects to link to a real, visible client record.
-- Existing legacy projects without clientId remain readable and editable by
-- their existing owner; new links must use the canonical client identity.

create or replace function private.aitask_project_client_is_valid(
  p_workspace_id text,
  p_client_id text,
  p_client_key text
)
...
set search_path = ''
as $$
  select nullif(trim(coalesce(p_client_id, '')), '') is not null
    and exists (
      select 1
      ...
      where client.workspace_id = p_workspace_id
        ...
        and client.client_key = lower(trim(coalesce(p_client_key, '')))
    )
    and private.aitask_can_view_client(p_workspace_id, lower(trim(coalesce(p_client_key, ''))));
$$;

revoke all on function private.aitask_project_client_is_valid(text, text, text)
  ...
  return case
    ...
    when p_entity_type = 'project' then
      ...
      and (
        ...
        and lower(trim(coalesce(p_new_data ->> 'clientName', ''))) = lower(trim(coalesce(p_old_data ->> 'clientName', '')))
