-- Let HODs (Staff-based custom role) add a new company, the same way Boss Koo
-- and Admins do, without granting them project, plan, or rename/delete rights.
--
-- 1. A dedicated `createClients` capability gates only client-profile creation.
-- 2. The frozen HOD role receives that capability for existing workspaces.
-- 3. Client creation is authorized server-side by the new capability, while
--    update/delete keep the existing assigned-client rule.
-- 4. A Staff member can read a company they created, so it does not disappear
--    from their workspace after the next sync.

create or replace function private.aitask_can_create_client(p_workspace_id text)
returns boolean
language sql
stable
security definer
...
as $$
  select private.aitask_is_super_admin(p_workspace_id)
    or private.aitask_is_admin(p_workspace_id)
    or (
      private.aitask_member_role(p_workspace_id) = 'Staff'
      and private.aitask_has_permission(p_workspace_id, 'createClients')
    );
$$;

revoke all on function private.aitask_can_create_client(text)
  from public, anon, authenticated;

-- Client visibility for Staff stays scoped, but the creator of a company can
-- always read it back (including before it has any tasks or projects).
...
revoke all on function private.aitask_can_view_client(text, text)
  from public, anon, authenticated;
...
create policy "members can insert authorized entities" on public.aitask_entities
  ...
  with check (
    ...
    or (entity_type = 'client'
      and private.aitask_can_create_client(workspace_id))
...
-- Grant the protected HOD role the company-creation capability for every
-- workspace. Its identity fields stay untouched.
update public.aitask_entities
set data = jsonb_set(data, '{permissions,createClients}', 'true'::jsonb, true),
    updated_at = now()
where entity_type = 'custom_role'
  and entity_id = 'system-hod'
  and coalesce((data -> 'permissions' ->> 'createClients'), 'false') <> 'true';
