-- Let Boss Koo, Admins, and HODs delete a company outright, mirroring the
-- Add companies capability. Ordinary Staff still cannot delete companies.
--
-- 1. A dedicated `deleteClients` capability gates only client deletion.
-- 2. The frozen HOD role receives that capability for existing workspaces.
-- 3. Client deletion is authorized server-side by the new capability, while
--    create and update keep their own rules.

create or replace function private.aitask_can_delete_client(p_workspace_id text)
returns boolean
language sql
stable
security definer
...
as $$
  select private.aitask_is_super_admin(p_workspace_id)
    ...
    or (
      ...
      and private.aitask_has_permission(p_workspace_id, 'deleteClients')
    );
$$;

revoke all on function private.aitask_can_delete_client(text)
...
create policy "members can delete authorized entities" on public.aitask_entities
  ...
  using (
    ...
    or (entity_type = 'client' and private.aitask_can_delete_client(workspace_id))
...
-- Grant the protected HOD role the company-deletion capability for every
-- workspace. Its identity fields stay untouched.
update public.aitask_entities
set data = jsonb_set(data, '{permissions,deleteClients}', 'true'::jsonb, true),
    updated_at = now()
where entity_type = 'custom_role'
  and entity_id = 'system-hod'
  and coalesce((data -> 'permissions' ->> 'deleteClients'), 'false') <> 'true';
