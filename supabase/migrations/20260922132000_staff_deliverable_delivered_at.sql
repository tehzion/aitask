-- Staff deliverable execution fields include deliveredAt.
--
-- `deriveServiceProgress` and `updateDeliverableStatus` set `deliveredAt` when a
-- deliverable becomes Delivered. A Staff member completing the deliverable's
-- tasks (without manageServiceCycles) was rejected by the Staff field allow-list
-- with "Only deliverable execution fields may be changed." and the change stayed
-- stuck as a retry. Add `deliveredAt` to the permitted deliverable fields.

CREATE OR REPLACE FUNCTION public.aitask_execute_service_command(p_workspace_id text, p_command_id uuid, p_command_type text, p_operations jsonb, p_expected_workspace_version bigint)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  for v_op in select value from jsonb_array_elements(p_operations) loop
    ...
    if v_role = 'Staff' and not private.aitask_has_permission(p_workspace_id, 'manageServiceCycles') and v_action = 'update' then
      ...
      elsif v_type = 'deliverable' and (v_old - array['status','taskIds','workflowGeneratedAt','workflowGenerationId','deliveredAt','updatedAt']::text[]) <> (v_new - array['status','taskIds','workflowGeneratedAt','workflowGenerationId','deliveredAt','updatedAt']::text[]) then
