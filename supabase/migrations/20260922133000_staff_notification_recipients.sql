-- Staff notifications may reach the owning Project Manager.
--
-- Task update notices no longer fan out to every Project Manager. A Staff
-- member's status/delete/create/comment/reminder notifications now target Boss
-- Koo (super admin) and the owning Project Manager (task assigned by-setter,
-- creator, or the owner of the task's client/project). Relax the Staff guard so
-- those recipients are allowed while still rejecting arbitrary members.
