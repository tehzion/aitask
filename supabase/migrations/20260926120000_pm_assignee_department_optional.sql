-- Project Managers may be assigned to tasks without department membership.
--
-- Project Managers are a supported department-less role. The final assignee
-- check in aitask_task_assignment_is_valid required the new assignee's
-- departments to contain the task department, so a PM with no departments could
-- never be a task's assignee (including self-assignment). PMs are
-- department-agnostic portfolio managers, so exempt them from that requirement;
-- Staff/HOD assignees still must belong to the task department.
...
begin
  ...
  return exists (
    ...
    where member.workspace_id = p_workspace_id
      ...
      and (member.role = 'Project Manager' or v_new_department = any(member.departments))
  );
end;
$$;
