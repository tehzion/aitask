...
--
-- A saved insert whose confirmation was lost (timeout, dropped response, or a
-- command id that changed on retry) is re-sent and previously failed with
-- CONFLICT "The record already exists." even though the row was already
-- written. Re-sending the identical insert is now a no-op success: the stored
-- row is kept and reported with its current version.

create or replace function public.aitask_execute_command_legacy(
  p_workspace_id text,
  p_command_id uuid,
  p_command_type text,
  p_operations jsonb
    ...
    if v_kind = 'entity' then
      ...
      if v_action = 'insert' and v_actual_version is not null then
        if v_old_data = v_new_data then
          -- Identical re-send of an insert already applied: skip it.
          continue;
        end if;
  ...
  loop
    ...
    if v_kind = 'entity' then
      ...
      if v_action = 'insert' then
        if v_old_data is not null and v_old_data = v_new_data then
          -- Already applied; keep the stored row and report its current version.
          select version, updated_at into v_row_version, v_row_updated_at
          from public.aitask_entities
          where workspace_id = p_workspace_id and entity_type = v_entity_type and entity_id = v_entity_id;
        else
...
revoke all on function public.aitask_execute_command_legacy(text, uuid, text, jsonb) from public, anon;
