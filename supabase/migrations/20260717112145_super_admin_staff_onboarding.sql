create index if not exists aitask_members_workspace_email_idx
on public.aitask_members (workspace_id, lower(coalesce(email, '')));

create index if not exists aitask_registration_workspace_email_idx
on public.aitask_entities (workspace_id, lower(coalesce(data ->> 'email', '')))
...
drop trigger if exists aitask_create_staff_registration on auth.users;
create trigger aitask_create_staff_registration
after insert on auth.users
for each row execute function private.aitask_create_staff_registration();
...
begin
  ...
  if v_member.id is null then
    ...
    insert into public.aitask_members (
      ...
      nullif(btrim(coalesce(p_client_name, '')), ''), false, false,
  ...
  insert into public.aitask_audit_events (
    ...
    array['auth_user_id', 'role', 'department'],
